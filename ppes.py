...
def write_input(atoms, folder, settings_dict, filename_label=None):
    ...
    if settings_dict["program"] == 'qe':
        ...
        if settings_dict.get("poscar_only", False):
            ...
            os.shutil.copyfile('INCAR', f'{folder}/INCAR')
            os.shutil.copyfile('KPOINTS', f'{folder}/KPOINTS')
            os.shutil.copyfile('POTCAR', f'{folder}/POTCAR')
...
def generate_sweep(upper, lower, settings_dict):
    ...
    if settings_dict.get("reference_distance", None) is not None:
        ...
        upper.positions[:,which_coord] = lower.positions[:,which_coord] + transl
    ...
    for dz in np.arange(range_sweep[0], range_sweep[1], step):
        ...
        write_input(new_atoms, folder, settings_dict)
